...
stored as parquet LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/products/';
...
CREATE EXTERNAL TABLE order_items (order_item_id int, order_item_order_id int, order_item_product_id int,order_item_quantity int, order_item_subtotal float,order_item_product_price float)
 stored as parquet LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/order_items';

drop table if exists orders;
CREATE EXTERNAL TABLE orders (order_id int, order_date bigint, order_customer_id int, order_status string) stored as parquet LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/orders';
...
stored as parquet LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/customers';

drop table if exists categories;
CREATE EXTERNAL TABLE categories (category_id int, category_department_id int, category_name string) stored as parquet
LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/categories';

drop table if exists departments;
CREATE EXTERNAL TABLE departments (department_id int, department_name string) stored as parquet
LOCATION 'adl://tobyse6.azuredatalakestore.net/Altus/retail_clickstream/order_tables/departments';
